...
var CmdApiapp = &commands.Command{
	...
	Short:     "Creates a IZIGo API application",
	Long: `
  The command 'api' creates a IZIGo API application.

  {{"Example:"|bold}}
      $ izi api [appname] [-tables=""] [-driver=mysql] [-conn=root:@tcp(127.0.0.1:3306)/test]
...
func main() {
	if izigo.BConfig.RunMode == "dev" {
		izigo.BConfig.WebConfig.DirectoryIndex = true
		izigo.BConfig.WebConfig.StaticDir["/swagger"] = "swagger"
	}
	izigo.Run()
}
`
...
func init() {
	ns := izigo.NewNamespace("/v1",
		izigo.NSNamespace("/object",
			izigo.NSInclude(
				&controllers.ObjectController{},
			),
		),
		izigo.NSNamespace("/user",
			izigo.NSInclude(
...
func init() {
	...
	u := User{"user_11111", "diepdt", "11111", Profile{"male", 20, "VietNam", "dotiendiep@gmail.com"}}
...
func TestGet(t *testing.T) {
	...
	izigo.Trace("testing", "TestGet", "Code[%d]\n%s", w.Code, w.Body.String())
...
func createAPI(cmd *commands.Command, args []string) int {
	...
	iziLogger.Log.Info("Creating API...")
	...
	if generate.SQLConn != "" {
		...
		iziLogger.Log.Infof("Using '%s' as 'driver'", generate.SQLDriver)
		iziLogger.Log.Infof("Using '%s' as 'conn'", generate.SQLConn)
	...
	iziLogger.Log.Success("New API successfully created!")
	return 0
}
