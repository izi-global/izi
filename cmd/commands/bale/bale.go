...
func runBale(cmd *commands.Command, args []string) int {
	...
	for _, p := range config.Conf.Bale.Dirs {
		if !utils.IsExist(p) {
			iziLogger.Log.Warnf("Skipped directory: %s", p)
			continue
		}
		iziLogger.Log.Infof("Packaging directory: %s", p)
	...
	if err != nil {
		iziLogger.Log.Fatalf("Failed to create file: %s", err)
	}
	defer fw.Close()

	_, err = fw.Write(buf.Bytes())
	if err != nil {
		iziLogger.Log.Fatalf("Failed to write data: %s", err)
	}

	iziLogger.Log.Success("Baled resources successfully!")
	return 0
}
...
func walkFn(resPath string, info os.FileInfo, _ error) error {
	...
	if err != nil {
		iziLogger.Log.Fatalf("Failed to read file: %s", err)
	}
