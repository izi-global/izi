...
var CmdDockerize = &commands.Command{
	CustomFlags: true,
	UsageLine:   "dockerize",
	Short:       "Generates a Dockerfile for your IZIGo application",
	Long: `Dockerize generates a Dockerfile for your IZIGo Web Application.
  The Dockerfile will compile, get the dependencies with {{"godep"|bold}}, and set the entrypoint.

  {{"Example:"|bold}}
    $ izi dockerize -expose="3000,80,25"
  `,
...
func dockerizeApp(cmd *commands.Command, args []string) int {
	if err := cmd.Flag.Parse(args); err != nil {
		iziLogger.Log.Fatalf("Error parsing flags: %v", err.Error())
	}

	iziLogger.Log.Info("Generating Dockerfile...")
...
func generateDockerfile(df Dockerfile) {
	t := template.Must(template.New("dockerBuildTemplate").Parse(dockerBuildTemplate)).Funcs(utils.IZIFuncMap())

	f, err := os.Create("Dockerfile")
	if err != nil {
		iziLogger.Log.Fatalf("Error writing Dockerfile: %v", err.Error())
	}
	defer utils.CloseFile(f)

	t.Execute(f, df)

	iziLogger.Log.Success("Dockerfile generated.")
}
