...
var cmdDlv = &commands.Command{
  ...
  To debug your application using Delve, use: {{"$ izi dlv" | bold}}
...
func runDlv(cmd *commands.Command, args []string) int {
	if err := cmd.Flag.Parse(args); err != nil {
		iziLogger.Log.Fatalf("Error while parsing flags: %v", err.Error())
	}
	...
	if err := loadPathsToWatch(&paths); err != nil {
		iziLogger.Log.Fatalf("Error while loading paths to watch: %v", err.Error())
	}
...
func startDelveDebugger(addr string, ch chan int) int {
	iziLogger.Log.Info("Starting Delve Debugger...")

	fp, err := buildDebug()
	if err != nil {
		iziLogger.Log.Fatalf("Error while building debug binary: %v", err)
	}
	defer os.Remove(fp)

	abs, err := filepath.Abs("./debug")
	if err != nil {
		iziLogger.Log.Fatalf("%v", err)
	}

	// Create and start the debugger server
	listener, err := net.Listen("tcp", addr)
	if err != nil {
		iziLogger.Log.Fatalf("Could not start listener: %s", err)
	}
	...
	if err := server.Run(); err != nil {
		iziLogger.Log.Fatalf("Could not start debugger server: %v", err)
	}
		...
		for {
			if val := <-ch; val == 0 {
				if _, err := client.Restart(); err != nil {
					utils.Notify("Error while restarting the client: "+err.Error(), "izi")
				} else {
					if verbose {
						utils.Notify("Delve Debugger Restarted", "izi")
					}
				}
			}
		}
	...
	status, err := term.Run()
	if err != nil {
		iziLogger.Log.Fatalf("Could not start Delve REPL: %v", err)
	}

	// Stop and kill the debugger server once user quits the REPL
	if err := server.Stop(true); err != nil {
		iziLogger.Log.Fatalf("Could not stop Delve server: %v", err)
	}
	return status
}
...
func startWatcher(paths []string, ch chan int) {
	...
	if err != nil {
		iziLogger.Log.Fatalf("Could not start the watcher: %v", err)
	}
	defer watcher.Close()
	...
	for _, path := range paths {
		if err := watcher.Add(path); err != nil {
			iziLogger.Log.Fatalf("Could not set a watch on path: %v", err)
		}
	}
	...
	for {
		...
		case evt := <-watcher.Events:
			...
			if build {
				go func() {
					if verbose {
						utils.Notify("Rebuilding application with the new changes", "izi")
					}
					...
					_, err := buildDebug()
					if err != nil {
						utils.Notify("Build Failed: "+err.Error(), "izi")
					} else {
