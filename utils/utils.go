...
func IsIZIGoProject(thePath string) bool {
	...
	go func() {
		filepath.Walk(thePath, func(fpath string, f os.FileInfo, err error) error {
			...
			if !f.IsDir() {
				...
				if len(hasIZIGoRegex.Find(data)) > 0 {
	...
	if err := <-c; err != nil {
		iziLogger.Log.Fatalf("Unable to walk '%s' tree: %s", thePath, err)
	}
...
func FormatSourceCode(filename string) {
	...
	if err := cmd.Run(); err != nil {
		iziLogger.Log.Warnf("Error while running gofmt: %s", err)
	}
}
...
// IZIFuncMap returns a FuncMap of functions used in different templates.
func IZIFuncMap() template.FuncMap {
...
func TmplToString(tmpl string, data interface{}) string {
	t := template.New("tmpl").Funcs(IZIFuncMap())
...
func Tmpl(text string, data interface{}) {
	...
	t := template.New("Usage").Funcs(IZIFuncMap())
...
func CheckEnv(appname string) (apppath, packpath string, err error) {
	...
	iziLogger.Log.Warn("You current workdir is not inside $GOPATH/src.")
	iziLogger.Log.Debugf("GOPATH: %s", FILE(), LINE(), gopath)
	...
	if _, e := os.Stat(apppath); !os.IsNotExist(e) {
		...
		iziLogger.Log.Errorf("Path '%s' already exists", apppath)
		return
	}
...
func GetFileModTime(path string) int64 {
	...
	f, err := os.Open(path)
	if err != nil {
		iziLogger.Log.Errorf("Failed to open file on '%s': %s", path, err)
	...
	if err != nil {
		iziLogger.Log.Errorf("Failed to get file stats: %s", err)
		return time.Now().Unix()
	}
