...
// IZILogger logs logging records to the specified io.Writer
type IZILogger struct {
	mu     sync.Mutex
	output io.Writer
}
...
var Log = GetIZILogger(os.Stdout)
...
// GetIZILogger initializes the logger instance with a NewColorWriter output
// and returns a singleton
func GetIZILogger(w io.Writer) *IZILogger {
	once.Do(func() {
		var (
			err             error
		...
		instance = &IZILogger{output: colors.NewColorWriter(w)}
	})
	return instance
}

// SetOutput sets the logger output destination
func (l *IZILogger) SetOutput(w io.Writer) {
	l.mu.Lock()
	defer l.mu.Unlock()
...
func (l *IZILogger) getLevelTag(level int) string {
	switch level {
	case levelFatal:
		return "FATAL   "
	case levelSuccess:
...
func (l *IZILogger) getColorLevel(level int) string {
	switch level {
	case levelCritical:
...
func (l *IZILogger) mustLog(level int, message string, args ...interface{}) {
	if level > logLevel {
		return
	}
	// Acquire the lock
...
func (l *IZILogger) mustLogDebug(message string, file string, line int, args ...interface{}) {
	if !debugMode {
		return
	}
